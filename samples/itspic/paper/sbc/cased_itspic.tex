...
A ipgm faz usa técnicas de processamento de imagem para detectar o estado atual da planta virtual, assim pegando informações como sensores e atuadores ativos. Além de coletar informações em tempo real, a biblioteca também e capaz de manipular a planta (ativando/desativando atuadores) simulando o uso de um teclado. Todas essas operações são feitas sem alterar qualquer parte do software ITS PLC. Todas elas foram feitas sem interferir com o processo do software ITS PLC.
...
As funções \textit{sup\_response\_step}, \textit{sup\_des\_step}, \textit{sup\_decisions\_step} e \textit{sup\_advance\_step} da Figura \ref{fig:piccontrol} são geradas automaticamente, enquanto o resto, como sequencias operacionais, são geradas manualmente. Isso dá a flexibilidade de alterar o modelo dos supervisores de forma eficiente, portanto que novos eventos controláveis não sejam introduzidos no sistema. Outra vantagem no uso deste compilador é a redução de falhas humanas, pois o processo de implementação dos supervisores pode ser extenso e tedioso.

\section{Resultados}
\label{sec:resultados}

Este trabalho conseguiu fazer a automação da planta virtual \textit{Palletizer} da ITS PLC, porém, apenas quando a planta se comporta bem. Um dos pontos interessantes do software ITS PLC é que ele executa simulações físicas na planta, assim reduzindo a diferente entre realidade e virtual. Essa característica na simulação implica que ao entrar uma caixa no sistema, por exemplo, ela pode vir em uma posição errada, ou cair de forma errada na esteira transportadora, com isso, gerando uma infinidade de estados da planta.
...
Este trabalho fez a implementação de um sistema capaz de se possibilitar a comunicação entre a planta virtual \textit{Palletizer} do software ITS PLC e um microcontrolador PIC simulado. O controle supervisório foi modelado usando a teoria de SED, e um compilador foi desenvolvido para gerar automaticamente instruções C seguindo os modelos dos supervisores.

Este trabalho conseguiu automatizar com sucesso a planta virtual, porém apenas quando ela é bem comportada. A introdução de elementos externos, ou instabilidade na planta, gera resultados indefinidos.
